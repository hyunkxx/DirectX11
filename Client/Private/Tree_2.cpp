#include "stdafx.h"
#include "..\Public\Tree_2.h"

#include "GameInstance.h"
#include "GameMode.h"

CTree_2::CTree_2(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
	: CGameObject(pDevice, pContext)
{
}

CTree_2::CTree_2(const CTree_2 & rhs)
	: CGameObject(rhs)
{
}

HRESULT CTree_2::Initialize_Prototype()
{
...
HRESULT CTree_2::Initialize(void * pArg)
{
...
void CTree_2::Tick(_double TimeDelta)
{
	__super::Tick(TimeDelta);
}

void CTree_2::LateTick(_double TimeDelta)
{
...
HRESULT CTree_2::Load_EditionColor(const _tchar * pEditionColorFilePath)
{
	...
	{
		MSG_BOX("Failed to Load Data in CTree_2 : EditionColor");
		return E_FAIL;
	}
...
HRESULT CTree_2::Add_Components()
{
	/* For.Com_Renderer*/
	...
	/* For.Com_Model */
	if (FAILED(__super::Add_Component(LEVEL_ANYWHERE, SMODEL::SMD_TREE_2,
...
HRESULT CTree_2::SetUp_ShaderResources()
{
...
HRESULT CTree_2::SetUp_ShadowShaderResources()
{
	if (nullptr == m_pShaderCom)
		return E_FAIL;
	CGameInstance* pGameInstance = CGameInstance::GetInstance();
...
CTree_2 * CTree_2::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
{
	CTree_2* pInstance = new CTree_2(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed to Create : CTree_2");
...
CGameObject * CTree_2::Clone(void * pArg)
{
	CTree_2* pInstance = new CTree_2(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed to Clone : CTree_2");
		Safe_Release(pInstance);
	}
