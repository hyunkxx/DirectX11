...
CM_Leilie::CM_Leilie(const CM_Leilie & rhs)
	: CCharacter(rhs)
{
}

HRESULT CM_Leilie::Initialize_Prototype()
{
...
HRESULT CM_Leilie::Initialize(void * pArg)
{
...
void CM_Leilie::Tick(_double TimeDelta)
{
...
void CM_Leilie::LateTick(_double TimeDelta)
{
...
HRESULT CM_Leilie::RenderShadow()
{
	if (false == m_bRender)
		return S_OK;
...
void CM_Leilie::RenderGUI()
{
}

HRESULT CM_Leilie::Add_Components()
{
	/* For.Com_Renderer*/
	...
	if (FAILED(__super::Add_Component(LEVEL_ANYWHERE, DMODEL::DMD_MONSTER_LEILIE,
...
HRESULT CM_Leilie::Init_States(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	...
	for (_int i = 0; i < IS_END; ++i)
	{
		...
		wsprintf(szBuffer, TEXT("../../Data/CharState/M_Leilie/Leilie_%d.state"), i);
...
void CM_Leilie::Shot_EffectKey(_tchar * szEffectTag/* szTag1*/, _uint EffectBoneID /* iInt0 */, _uint iEffectTypeID, _bool bTracking/*iInt1*/)
{
...
void CM_Leilie::Shot_MissileKey(_uint iMissilePoolID, _uint iEffectBoneID)
{
...
void CM_Leilie::Shot_PriorityKey(_uint iLeavePriority)
{
	m_tCurState.iLeavePriority = iLeavePriority;
}

void CM_Leilie::SetUp_State()
{
	// 키 리셋
...
void CM_Leilie::Change_Target(CCharacter * pActiveCharacter)
{
	m_pTarget = pActiveCharacter;
	m_pTargetTransform = m_pTarget->GetTransform();
}

void CM_Leilie::Find_Target()
{
...
void CM_Leilie::Init_AttackInfos()
{
...
void CM_Leilie::Init_Missiles()
{
	...
	tMissilePoolDesc.tMissileDesc.HitInterval = 0.0;
	tMissilePoolDesc.tMissileDesc.LifeTime = 3.0;
	tMissilePoolDesc.tMissileDesc.iAttackInfoID = ATK_ATTACK_02_1;
	tMissilePoolDesc.tMissileDesc.fExtents = 0.4f;
	...
	m_MissilePools[MISS_ATTACK_02_1] = CMissilePool::Create(m_pDevice, m_pContext, XMVectorSet(0.f, 0.f, 0.f, 0.f), &tMissilePoolDesc);
	m_MissileRotAngles[MISS_ATTACK_02_1] = _float3(0.f, 0.f, 0.f);
	...
	m_MissilePools[MISS_ATTACK_02_2] = CMissilePool::Create(m_pDevice, m_pContext, XMVectorSet(0.f, 0.f, 0.f, 0.f), &tMissilePoolDesc);
	m_MissileRotAngles[MISS_ATTACK_02_2] = _float3(0.f, 0.f, 0.f);
...
void CM_Leilie::Init_AnimSystem()
{
	for (auto& pAnim : m_pModelCom->Get_Animations())
	{
		...
		for (auto& pChannel : pAnim->Get_Channels())
		{
			...
			CBone* pBone = m_pModelCom->Get_BonePtr(pChannel->Get_TargetBoneID());


			if (!lstrcmp(szAnimName, TEXT("MO1LeilieMd00301.ao|StandUp")) ||
				!lstrcmp(szAnimName, TEXT("MO1LeilieMd00301.ao|Behit_S_Press")))
			{
				if (!lstrcmp(szChannelName, TEXT("Skirt_bone10")) ||
					!lstrcmp(szChannelName, TEXT("Skirt_bone15")) ||
					!lstrcmp(szChannelName, TEXT("Skirt_bone01")) ||
					!lstrcmp(szChannelName, TEXT("Skirt_bone05")) ||
					true == pBone->Is_ChildOf(TEXT("Skirt_bone10")) ||
					true == pBone->Is_ChildOf(TEXT("Skirt_bone15")) ||
					true == pBone->Is_ChildOf(TEXT("Skirt_bone01")) ||
					true == pBone->Is_ChildOf(TEXT("Skirt_bone05")))
				{
...
void CM_Leilie::Apply_CoolTime(_double TimeDelta)
{
...
HRESULT CM_Leilie::SetUp_ShaderResources()
{
...
HRESULT CM_Leilie::Setup_ShadowShaderResource()
{
	if (nullptr == m_pShaderCom)
		return E_FAIL;
	CGameInstance* pGameInstance = CGameInstance::GetInstance();
...
void CM_Leilie::Select_State(_double TimeDelta)
{
	...
	switch (iCurFrameAI)
	{
	case Client::CM_Leilie::AI_IDLE:
		m_Scon.iNextState = AI_IDLE;
		break;
	case Client::CM_Leilie::AI_ATTACK:
		if (0.0 == m_StateCoolTimes[IS_ATTACK03])
			m_Scon.iNextState = IS_ATTACK03;
		else if (0.0 == m_StateCoolTimes[IS_ATTACK02])
			m_Scon.iNextState = IS_ATTACK02;
		else
			m_Scon.iNextState = IS_ATTACK01_1;
		break;
	case Client::CM_Leilie::AI_CHASE:
		m_Scon.iNextState = IS_RUN;
		break;
	case Client::CM_Leilie::AI_STAY:
...
void CM_Leilie::Tick_State(_double TimeDelta)
{
	//
	...
	if (true == m_Scon.bAnimFinished)
	{
		...
		// 공격 행동 시
		if (IS_ATTACK01_3 == m_Scon.iCurState ||
...
void CM_Leilie::On_Hit(CCharacter * pChar, TAGATTACK * pAttackInfo, _float fAttackPoint, _float3 * pEffPos, _float fCritRate, _float fCritDMG)
{
	...
	if (0.f >= m_tMonsterInfo.fCurHP)
	{
		...
		if (!bDie)
		{
			...
			if (!pEchoSystem->GetEcho(CEchoSystem::EC_IMPULSE_PREDATOR).bActive)
			{
				pEchoSystem->SetActiveEcho(CEchoSystem::EC_IMPULSE_PREDATOR);
				CItem::ITEM_DESC item = pDB->GetItemData(ITEM::E_IMPULSE);
...
void CM_Leilie::Update_EffectBones()
{
...
CM_Leilie * CM_Leilie::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
{
	CM_Leilie* pInstance = new CM_Leilie(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed to Create : CM_Leilie");
...
CGameObject * CM_Leilie::Clone(void * pArg)
{
	CM_Leilie* pInstance = new CM_Leilie(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed to Clone : CM_Leilie");
		Safe_Release(pInstance);
	}
...
void CM_Leilie::OnCollisionEnter(CCollider * src, CCollider * dest)
{
	CGameMode* pGM = CGameMode::GetInstance();
...
void CM_Leilie::OnCollisionStay(CCollider * src, CCollider * dest)
{
}

void CM_Leilie::OnCollisionExit(CCollider * src, CCollider * dest)
{
}
