...
CE_FHuxiuxiu::CE_FHuxiuxiu(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
	: CCharacter(pDevice, pContext)
{
}

CE_FHuxiuxiu::CE_FHuxiuxiu(const CE_FHuxiuxiu & rhs)
	: CCharacter(rhs)
{
}

HRESULT CE_FHuxiuxiu::Initialize_Prototype()
{
...
HRESULT CE_FHuxiuxiu::Initialize(void * pArg)
{
	...
	m_pEchoSystem->Register_EchoPtr(CEchoSystem::EC_WHOOSH, this);
...
void CE_FHuxiuxiu::PreTick(_double TimeDelta)
{
...
void CE_FHuxiuxiu::Tick(_double TimeDelta)
{
...
void CE_FHuxiuxiu::LateTick(_double TimeDelta)
{
...
HRESULT CE_FHuxiuxiu::RenderShadow()
{
	if (false == m_bRender)
		return S_OK;
...
void CE_FHuxiuxiu::RenderGUI()
{
}

HRESULT CE_FHuxiuxiu::Add_Components()
{
	/* For.Com_Renderer*/
	...
	if (FAILED(__super::Add_Component(LEVEL_STATIC, DMODEL::DMD_ECHO_FHUXIUXIU,
...
HRESULT CE_FHuxiuxiu::Init_State(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	ZeroMemory(&m_tCurState, sizeof(SINGLESTATE));

	// 로드하는 코드
	_tchar szBuffer[MAX_PATH];
	wsprintf(szBuffer, TEXT("../../Data/CharState/Echos/FHuxiuxiu_8.state"));
...
void CE_FHuxiuxiu::Release_State()
{
	if (0 == m_tCurState.iKeyCount)
		return;
...
HRESULT CE_FHuxiuxiu::Reload_Components()
{
	Release_Reloadable();
...
void CE_FHuxiuxiu::Release_Reloadable()
{
...
void CE_FHuxiuxiu::Shot_EffectKey(_tchar * szEffectTag/* szTag1*/, _uint EffectBoneID /* iInt0 */, _uint iEffectTypeID, _bool bTracking/*iInt1*/)
{
...
void CE_FHuxiuxiu::Shot_MissileKey(_uint iMissilePoolID, _uint iEffectBoneID)
{
	_vector vInitPos;
	if (0 != iEffectBoneID)
	{
...
void CE_FHuxiuxiu::Shot_OBBKey(_bool bOBB, _uint iAttackInfoID)
{
	m_pAttackCollider->SetActive(bOBB);
	//m_iCurAttackID = iAttackInfoID;
}

void CE_FHuxiuxiu::Shot_DissolveKey(_bool bDissolveType, _float fDissolveSpeed)
{
...
void CE_FHuxiuxiu::Shot_Echo(CTransform * pTransform, CCharacter * pTarget, _uint iNaviCellID)
{
	SetState(ACTIVE);
...
void CE_FHuxiuxiu::Release_Missiles()
{
	if (nullptr != m_MissilePool)
		Safe_Release(m_MissilePool);
}

HRESULT CE_FHuxiuxiu::SetUp_ShaderResources()
{
	if (nullptr == m_pShaderCom)
		return E_FAIL;
	CGameInstance* pGameInstance = CGameInstance::GetInstance();
...
HRESULT CE_FHuxiuxiu::Setup_ShadowShaderResource()
{
	if (nullptr == m_pShaderCom)
		return E_FAIL;
	CGameInstance* pGameInstance = CGameInstance::GetInstance();
...
void CE_FHuxiuxiu::Tick_State(_double TimeDelta)
{
	//
...
HRESULT CE_FHuxiuxiu::Init_EffectBones()
{
...
void CE_FHuxiuxiu::Update_EffectBones()
{
...
CE_FHuxiuxiu * CE_FHuxiuxiu::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
{
	CE_FHuxiuxiu* pInstance = new CE_FHuxiuxiu(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed to Create : CE_FHuxiuxiu");
...
CGameObject * CE_FHuxiuxiu::Clone(void * pArg)
{
	CE_FHuxiuxiu* pInstance = new CE_FHuxiuxiu(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed to Clone : CE_FHuxiuxiu");
		Safe_Release(pInstance);
	}
...
void CE_FHuxiuxiu::OnCollisionEnter(CCollider * src, CCollider * dest)
{
	CGameMode* pGM = CGameMode::GetInstance();
...
void CE_FHuxiuxiu::OnCollisionStay(CCollider * src, CCollider * dest)
{
	CCharacter* pOpponent = dynamic_cast<CCharacter*>(dest->GetOwner());
...
void CE_FHuxiuxiu::OnCollisionExit(CCollider * src, CCollider * dest)
{
}
