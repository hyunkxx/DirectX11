...
CE_Fenglie::CE_Fenglie(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
	: CCharacter(pDevice, pContext)
{
}

CE_Fenglie::CE_Fenglie(const CE_Fenglie & rhs)
	: CCharacter(rhs)
{
}

HRESULT CE_Fenglie::Initialize_Prototype()
{
...
HRESULT CE_Fenglie::Initialize(void * pArg)
{
	...
	m_pEchoSystem->Register_EchoPtr(CEchoSystem::EC_PNENUMA_PREDATOR, this);
...
void CE_Fenglie::PreTick(_double TimeDelta)
{
...
void CE_Fenglie::Tick(_double TimeDelta)
{
...
void CE_Fenglie::LateTick(_double TimeDelta)
{
...
HRESULT CE_Fenglie::RenderShadow()
{
	if (false == m_bRender)
		return S_OK;
...
void CE_Fenglie::RenderGUI()
{
}

HRESULT CE_Fenglie::Add_Components()
{
	/* For.Com_Renderer*/
	...
	if (FAILED(__super::Add_Component(LEVEL_STATIC, DMODEL::DMD_ECHO_FENGLIE,
...
HRESULT CE_Fenglie::Init_State(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	ZeroMemory(&m_tCurState, sizeof(SINGLESTATE));

	// 로드하는 코드
	_tchar szBuffer[MAX_PATH];
	wsprintf(szBuffer, TEXT("../../Data/CharState/Echos/Fenglie_8.state"));
...
HRESULT CE_Fenglie::Reload_Components()
{
	Release_Reloadable();
...
void CE_Fenglie::Release_Reloadable()
{
...
void CE_Fenglie::Shot_EffectKey(_tchar * szEffectTag/* szTag1*/, _uint EffectBoneID /* iInt0 */, _uint iEffectTypeID, _bool bTracking/*iInt1*/)
{
...
void CE_Fenglie::Shot_MissileKey(_uint iMissilePoolID, _uint iEffectBoneID)
{
	_vector vInitPos;
	if (0 != iEffectBoneID)
	{
...
void CE_Fenglie::Shot_OBBKey(_bool bOBB, _uint iAttackInfoID)
{
	m_pAttackCollider->SetActive(bOBB);
	//m_iCurAttackID = iAttackInfoID;
}

void CE_Fenglie::Shot_DissolveKey(_bool bDissolveType, _float fDissolveSpeed)
{
...
void CE_Fenglie::Shot_Echo(CTransform * pTransform, CCharacter * pTarget, _uint iNaviCellID)
{
	SetState(ACTIVE);
...
void CE_Fenglie::Change_Target(CCharacter * pActiveCharacter)
{
	m_pTarget = pActiveCharacter;
	m_pTargetTransform = m_pTarget->GetTransform();
}

void CE_Fenglie::Init_AttackInfos()
{
...
HRESULT CE_Fenglie::Setup_ShadowShaderResource()
{
	if (nullptr == m_pShaderCom)
		return E_FAIL;
	CGameInstance* pGameInstance = CGameInstance::GetInstance();
...
void CE_Fenglie::Tick_State(_double TimeDelta)
{
	//
...
HRESULT CE_Fenglie::Init_EffectBones()
{
...
void CE_Fenglie::Update_EffectBones()
{
...
CE_Fenglie * CE_Fenglie::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
{
	CE_Fenglie* pInstance = new CE_Fenglie(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed to Create : CE_Fenglie");
...
CGameObject * CE_Fenglie::Clone(void * pArg)
{
	CE_Fenglie* pInstance = new CE_Fenglie(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed to Clone : CE_Fenglie");
		Safe_Release(pInstance);
	}
...
void CE_Fenglie::OnCollisionEnter(CCollider * src, CCollider * dest)
{
	CGameMode* pGM = CGameMode::GetInstance();
...
void CE_Fenglie::OnCollisionStay(CCollider * src, CCollider * dest)
{
	CCharacter* pOpponent = dynamic_cast<CCharacter*>(dest->GetOwner());
...
void CE_Fenglie::OnCollisionExit(CCollider * src, CCollider * dest)
{
}
