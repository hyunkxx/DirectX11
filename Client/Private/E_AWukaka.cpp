...
CE_AWukaka::CE_AWukaka(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
	: CCharacter(pDevice, pContext)
{
}

CE_AWukaka::CE_AWukaka(const CE_AWukaka & rhs)
	: CCharacter(rhs)
{
}

HRESULT CE_AWukaka::Initialize_Prototype()
{
...
HRESULT CE_AWukaka::Initialize(void * pArg)
{
	...
	m_pEchoSystem->Register_EchoPtr(CEchoSystem::EC_TICTAC, this);
...
void CE_AWukaka::PreTick(_double TimeDelta)
{
...
void CE_AWukaka::Tick(_double TimeDelta)
{
...
void CE_AWukaka::LateTick(_double TimeDelta)
{
...
HRESULT CE_AWukaka::RenderShadow()
{
	if (false == m_bRender)
		return S_OK;
...
void CE_AWukaka::RenderGUI()
{
}

HRESULT CE_AWukaka::Add_Components()
{
	/* For.Com_Renderer*/
	...
	if (FAILED(__super::Add_Component(LEVEL_STATIC, DMODEL::DMD_ECHO_AWUKAKA,
...
HRESULT CE_AWukaka::Init_State(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	ZeroMemory(&m_tCurState, sizeof(SINGLESTATE));

	// 로드하는 코드
	_tchar szBuffer[MAX_PATH];
	wsprintf(szBuffer, TEXT("../../Data/CharState/Echos/AWukaka_9.state"));
...
HRESULT CE_AWukaka::Reload_Components()
{
	Release_Reloadable();
...
void CE_AWukaka::Release_Reloadable()
{
...
void CE_AWukaka::Shot_EffectKey(_tchar * szEffectTag/* szTag1*/, _uint EffectBoneID /* iInt0 */, _uint iEffectTypeID, _bool bTracking/*iInt1*/)
{
...
void CE_AWukaka::Shot_MissileKey(_uint iMissilePoolID, _uint iEffectBoneID)
{
	_vector vInitPos;

	m_pCamMovement->StartWave(3);
...
void CE_AWukaka::Shot_OBBKey(_bool bOBB, _uint iAttackInfoID)
{
	m_pAttackCollider->SetActive(bOBB);
	//m_iCurAttackID = iAttackInfoID;
}

void CE_AWukaka::Shot_DissolveKey(_bool bDissolveType, _float fDissolveSpeed)
{
...
void CE_AWukaka::Shot_Echo(CTransform * pTransform, CCharacter * pTarget, _uint iNaviCellID)
{
	SetState(ACTIVE);
...
void CE_AWukaka::Change_Target(CCharacter * pActiveCharacter)
{
	m_pTarget = pActiveCharacter;
	m_pTargetTransform = m_pTarget->GetTransform();
}

void CE_AWukaka::Init_AttackInfos()
{
...
void CE_AWukaka::Release_Missiles()
{
	if (nullptr != m_MissilePool)
		Safe_Release(m_MissilePool);
}

HRESULT CE_AWukaka::SetUp_ShaderResources()
{
	if (nullptr == m_pShaderCom)
		return E_FAIL;
	CGameInstance* pGameInstance = CGameInstance::GetInstance();
...
HRESULT CE_AWukaka::Setup_ShadowShaderResource()
{
	if (nullptr == m_pShaderCom)
		return E_FAIL;
	CGameInstance* pGameInstance = CGameInstance::GetInstance();
...
void CE_AWukaka::Tick_State(_double TimeDelta)
{
	//
...
HRESULT CE_AWukaka::Init_EffectBones()
{
...
void CE_AWukaka::Update_EffectBones()
{
...
CE_AWukaka * CE_AWukaka::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
{
	CE_AWukaka* pInstance = new CE_AWukaka(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed to Create : CE_AWukaka");
...
CGameObject * CE_AWukaka::Clone(void * pArg)
{
	CE_AWukaka* pInstance = new CE_AWukaka(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed to Clone : CE_AWukaka");
		Safe_Release(pInstance);
	}
...
void CE_AWukaka::OnCollisionEnter(CCollider * src, CCollider * dest)
{
	CGameMode* pGM = CGameMode::GetInstance();
...
void CE_AWukaka::OnCollisionStay(CCollider * src, CCollider * dest)
{
	CCharacter* pOpponent = dynamic_cast<CCharacter*>(dest->GetOwner());
...
void CE_AWukaka::OnCollisionExit(CCollider * src, CCollider * dest)
{
}
