...
void CM_Huojin::On_Hit(CCharacter * pChar, TAGATTACK * pAttackInfo, _float fAttackPoint, _float3 * pEffPos, _float fCritRate, _float fCritDMG)
{
	// 피격 이펙트 출력
	...
	if (0.f >= m_tMonsterInfo.fCurHP)
	{
		...
		{
			bDie = true;
			if (!pEchoSystem->GetEcho(CEchoSystem::EC_FUSHION_WARRIOR).bActive)
			{
				pEchoSystem->SetActiveEcho(CEchoSystem::EC_FUSHION_WARRIOR);
