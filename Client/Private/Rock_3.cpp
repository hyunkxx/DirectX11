...
void CRock_3::LateTick(_double TimeDelta)
{
...
HRESULT CRock_3::Load_DiffuseTexID(const _tchar * pDiffuseTexIDFilePath)
{
	...
	{
		MSG_BOX("Failed to Load Data in CRock_3 : DiffuseTexID");
		return E_FAIL;
	}
...
HRESULT CRock_3::Add_Components()
{
	/* For.Com_Renderer*/
	...
	/* For.Com_Model */
	if (FAILED(__super::Add_Component(LEVEL_ANYWHERE, SMODEL::SMD_ROCK_3,
...
HRESULT CRock_3::SetUp_ShadowShaderResources()
{
	if (nullptr == m_pShaderCom)
		return E_FAIL;
	CGameInstance* pGameInstance = CGameInstance::GetInstance();
...
CRock_3 * CRock_3::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
{
	...
	{
		MSG_BOX("Failed to Create : CRock_3");
...
CGameObject * CRock_3::Clone(void * pArg)
{
	...
	{
		MSG_BOX("Failed to Clone : CRock_3");
		Safe_Release(pInstance);
	}
