#include "stdafx.h"
#include "..\Public\Rock_9.h"

#include "GameInstance.h"
#include "GameMode.h"

CRock_9::CRock_9(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
	: CGameObject(pDevice, pContext)
{
}

CRock_9::CRock_9(const CRock_9 & rhs)
	: CGameObject(rhs)
{
}

HRESULT CRock_9::Initialize_Prototype()
{
...
HRESULT CRock_9::Initialize(void * pArg)
{
...
void CRock_9::Tick(_double TimeDelta)
{
	__super::Tick(TimeDelta);
}

void CRock_9::LateTick(_double TimeDelta)
{
...
HRESULT CRock_9::Load_DiffuseTexID(const _tchar * pDiffuseTexIDFilePath)
{
	...
	{
		MSG_BOX("Failed to Load Data in CRock_9 : DiffuseTexID");
		return E_FAIL;
	}
...
HRESULT CRock_9::Add_Components()
{
	/* For.Com_Renderer*/
	...
	/* For.Com_Model */
	if (FAILED(__super::Add_Component(LEVEL_ANYWHERE, SMODEL::SMD_ROCK_9,
...
HRESULT CRock_9::SetUp_ShaderResources()
{
...
HRESULT CRock_9::SetUp_ShadowShaderResources()
{
	if (nullptr == m_pShaderCom)
		return E_FAIL;
	CGameInstance* pGameInstance = CGameInstance::GetInstance();
...
CRock_9 * CRock_9::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
{
	CRock_9* pInstance = new CRock_9(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed to Create : CRock_9");
...
CGameObject * CRock_9::Clone(void * pArg)
{
	CRock_9* pInstance = new CRock_9(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed to Clone : CRock_9");
		Safe_Release(pInstance);
	}
