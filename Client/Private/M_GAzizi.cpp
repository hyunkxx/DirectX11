...
HRESULT CM_GAzizi::Initialize(void * pArg)
{
	...
	// CharInfo 초기화
	lstrcpy(m_tMonsterInfo.szName, TEXT("ZigZag"));
	...
	m_tMonsterInfo.fAttack = 85.f;
	m_tMonsterInfo.fDefense = 65.f;
...
void CM_GAzizi::On_Hit(CCharacter * pChar, TAGATTACK * pAttackInfo, _float fAttackPoint, _float3 * pEffPos, _float fCritRate, _float fCritDMG)
{
	// 피격 이펙트 출력
	...
	if (0.f >= m_tMonsterInfo.fCurHP)
	{
		...
		{
			bDie = true;
			if (!pEchoSystem->GetEcho(CEchoSystem::EC_ZIGZAG).bActive)
			{
				pEchoSystem->SetActiveEcho(CEchoSystem::EC_ZIGZAG);
				CItem::ITEM_DESC item = pDB->GetItemData(ITEM::E_ZIGZAG);
