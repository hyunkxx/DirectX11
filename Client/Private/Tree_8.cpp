#include "stdafx.h"
#include "..\Public\Tree_8.h"

#include "GameInstance.h"
#include "GameMode.h"

CTree_8::CTree_8(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
	: CGameObject(pDevice, pContext)
{
}

CTree_8::CTree_8(const CTree_8 & rhs)
	: CGameObject(rhs)
{
}

HRESULT CTree_8::Initialize_Prototype()
{
...
HRESULT CTree_8::Initialize(void * pArg)
{
...
void CTree_8::Tick(_double TimeDelta)
{
	__super::Tick(TimeDelta);
}

void CTree_8::LateTick(_double TimeDelta)
{
...
HRESULT CTree_8::Load_EditionColor(const _tchar * pEditionColorFilePath)
{
	...
	{
		MSG_BOX("Failed to Load Data in CTree_8 : EditionColor");
		return E_FAIL;
	}
...
HRESULT CTree_8::Add_Components()
{
	/* For.Com_Renderer*/
	...
	/* For.Com_Model */
	if (FAILED(__super::Add_Component(LEVEL_ANYWHERE, SMODEL::SMD_TREE_8,
...
HRESULT CTree_8::SetUp_ShaderResources()
{
...
HRESULT CTree_8::SetUp_ShadowShaderResources()
{
	if (nullptr == m_pShaderCom)
		return E_FAIL;
	CGameInstance* pGameInstance = CGameInstance::GetInstance();
...
CTree_8 * CTree_8::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
{
	CTree_8* pInstance = new CTree_8(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed to Create : CTree_8");
...
CGameObject * CTree_8::Clone(void * pArg)
{
	CTree_8* pInstance = new CTree_8(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed to Clone : CTree_8");
		Safe_Release(pInstance);
	}
