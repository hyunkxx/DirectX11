...
void CM_Qunjing::On_Hit(CCharacter * pChar, TAGATTACK * pAttackInfo, _float fAttackPoint, _float3 * pEffPos, _float fCritRate, _float fCritDMG)
{
	...
	if (0.f >= m_tMonsterInfo.fCurHP)
	{
		...
		if (!bDie)
		{
			...
			if (!pEchoSystem->GetEcho(CEchoSystem::EC_ELITE_STONE).bActive)
			{
				pEchoSystem->SetActiveEcho(CEchoSystem::EC_ELITE_STONE);
				CItem::ITEM_DESC item = pDB->GetItemData(ITEM::E_STONE);
