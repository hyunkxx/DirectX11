#include "stdafx.h"
#include "..\Public\Rock_5.h"

#include "GameInstance.h"
#include "GameMode.h"

CRock_5::CRock_5(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
	: CGameObject(pDevice, pContext)
{
}

CRock_5::CRock_5(const CRock_5 & rhs)
	: CGameObject(rhs)
{
}

HRESULT CRock_5::Initialize_Prototype()
{
...
HRESULT CRock_5::Initialize(void * pArg)
{
...
void CRock_5::Tick(_double TimeDelta)
{
	__super::Tick(TimeDelta);
}

void CRock_5::LateTick(_double TimeDelta)
{
...
HRESULT CRock_5::Load_DiffuseTexID(const _tchar * pDiffuseTexIDFilePath)
{
	...
	{
		MSG_BOX("Failed to Load Data in CRock_5 : DiffuseTexID");
		return E_FAIL;
	}
...
HRESULT CRock_5::Add_Components()
{
	/* For.Com_Renderer*/
	...
	/* For.Com_Model */
	if (FAILED(__super::Add_Component(LEVEL_ANYWHERE, SMODEL::SMD_ROCK_5,
...
HRESULT CRock_5::SetUp_ShaderResources()
{
...
HRESULT CRock_5::SetUp_ShadowShaderResources()
{
	if (nullptr == m_pShaderCom)
		return E_FAIL;
	CGameInstance* pGameInstance = CGameInstance::GetInstance();
...
CRock_5 * CRock_5::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
{
	CRock_5* pInstance = new CRock_5(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed to Create : CRock_5");
...
CGameObject * CRock_5::Clone(void * pArg)
{
	CRock_5* pInstance = new CRock_5(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed to Clone : CRock_5");
		Safe_Release(pInstance);
	}
