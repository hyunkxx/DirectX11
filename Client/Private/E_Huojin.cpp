...
CE_Huojin::CE_Huojin(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
	: CCharacter(pDevice, pContext)
{
}

CE_Huojin::CE_Huojin(const CE_Huojin & rhs)
	: CCharacter(rhs)
{
}

HRESULT CE_Huojin::Initialize_Prototype()
{
...
HRESULT CE_Huojin::Initialize(void * pArg)
{
	...
	m_pEchoSystem->Register_EchoPtr(CEchoSystem::EC_FUSHION_WARRIOR, this);
...
void CE_Huojin::PreTick(_double TimeDelta)
{
...
void CE_Huojin::Tick(_double TimeDelta)
{
...
void CE_Huojin::LateTick(_double TimeDelta)
{
...
HRESULT CE_Huojin::RenderShadow()
{
	if (false == m_bRender)
		return S_OK;
...
void CE_Huojin::RenderGUI()
{
}

HRESULT CE_Huojin::Add_Components()
{
	/* For.Com_Renderer*/
	...
	if (FAILED(__super::Add_Component(LEVEL_STATIC, DMODEL::DMD_ECHO_HUOJIN,
...
HRESULT CE_Huojin::Init_State(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	ZeroMemory(&m_tCurState, sizeof(SINGLESTATE));

	// 로드하는 코드
	_tchar szBuffer[MAX_PATH];
	wsprintf(szBuffer, TEXT("../../Data/CharState/Echos/Huojin_8.state"));
...
HRESULT CE_Huojin::Reload_Components()
{
	Release_Reloadable();
...
void CE_Huojin::Release_Reloadable()
{
...
void CE_Huojin::Shot_EffectKey(_tchar * szEffectTag/* szTag1*/, _uint EffectBoneID /* iInt0 */, _uint iEffectTypeID, _bool bTracking/*iInt1*/)
{
...
//
//void CE_Huojin::Shot_MissileKey(_uint iMissilePoolID, _uint iEffectBoneID)
//{
...
void CE_Huojin::Shot_OBBKey(_bool bOBB, _uint iAttackInfoID)
{
	m_pAttackCollider->SetActive(bOBB);
	//m_iCurAttackID = iAttackInfoID;
}

void CE_Huojin::Shot_DissolveKey(_bool bDissolveType, _float fDissolveSpeed)
{
...
void CE_Huojin::Shot_Echo(CTransform * pTransform, CCharacter * pTarget, _uint iNaviCellID)
{
	SetState(ACTIVE);
...
void CE_Huojin::Change_Target(CCharacter * pActiveCharacter)
{
	m_pTarget = pActiveCharacter;
	m_pTargetTransform = m_pTarget->GetTransform();
}

void CE_Huojin::Init_AttackInfos()
{
	ZeroMemory(&m_AttackInfo, sizeof TAGATTACK);

	m_AttackInfo.fDamageFactor = 2.3f;
	m_AttackInfo.eHitIntensity = HIT_FLY;
	m_AttackInfo.eElementType = ELMT_FUSION;
	m_AttackInfo.fSPGain = 0.f;
	m_AttackInfo.fTPGain = 0.f;
	m_AttackInfo.iHitEffectID = 0;
	lstrcpy(m_AttackInfo.szHitEffectTag, TEXT("M_Red_Hit"));
}
//
//void CE_Huojin::Init_Missiles()
//{
//	// Attack01
...
HRESULT CE_Huojin::SetUp_ShaderResources()
{
	if (nullptr == m_pShaderCom)
		return E_FAIL;
	CGameInstance* pGameInstance = CGameInstance::GetInstance();
...
HRESULT CE_Huojin::Setup_ShadowShaderResource()
{
	if (nullptr == m_pShaderCom)
		return E_FAIL;
	CGameInstance* pGameInstance = CGameInstance::GetInstance();
...
void CE_Huojin::Tick_State(_double TimeDelta)
{
	//
...
HRESULT CE_Huojin::Init_EffectBones()
{
...
void CE_Huojin::Update_EffectBones()
{
...
CE_Huojin * CE_Huojin::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
{
	CE_Huojin* pInstance = new CE_Huojin(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed to Create : CE_Huojin");
...
CGameObject * CE_Huojin::Clone(void * pArg)
{
	CE_Huojin* pInstance = new CE_Huojin(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed to Clone : CE_Huojin");
		Safe_Release(pInstance);
	}
...
void CE_Huojin::OnCollisionEnter(CCollider * src, CCollider * dest)
{
	CGameMode* pGM = CGameMode::GetInstance();
...
void CE_Huojin::OnCollisionStay(CCollider * src, CCollider * dest)
{
	CCharacter* pOpponent = dynamic_cast<CCharacter*>(dest->GetOwner());
...
void CE_Huojin::OnCollisionExit(CCollider * src, CCollider * dest)
{
}
