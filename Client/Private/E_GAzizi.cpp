...
CE_GAzizi::CE_GAzizi(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
	: CCharacter(pDevice, pContext)
{
}

CE_GAzizi::CE_GAzizi(const CE_GAzizi & rhs)
	: CCharacter(rhs)
{
}

HRESULT CE_GAzizi::Initialize_Prototype()
{
...
HRESULT CE_GAzizi::Initialize(void * pArg)
{
	...
	m_pEchoSystem->Register_EchoPtr(CEchoSystem::EC_ZIGZAG, this);
...
void CE_GAzizi::PreTick(_double TimeDelta)
{
...
void CE_GAzizi::Tick(_double TimeDelta)
{
...
void CE_GAzizi::LateTick(_double TimeDelta)
{
...
HRESULT CE_GAzizi::RenderShadow()
{
	if (false == m_bRender)
		return S_OK;
...
void CE_GAzizi::RenderGUI()
{
}

HRESULT CE_GAzizi::Add_Components()
{
	/* For.Com_Renderer*/
	...
	if (FAILED(__super::Add_Component(LEVEL_STATIC, DMODEL::DMD_ECHO_GAZIZI,
...
HRESULT CE_GAzizi::Init_State(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	ZeroMemory(&m_tCurState, sizeof(SINGLESTATE));

	// 로드하는 코드
	_tchar szBuffer[MAX_PATH];
	wsprintf(szBuffer, TEXT("../../Data/CharState/Echos/GAzizi_8.state"));
...
HRESULT CE_GAzizi::Reload_Components()
{
	Release_Reloadable();
...
void CE_GAzizi::Release_Reloadable()
{
...
void CE_GAzizi::Shot_EffectKey(_tchar * szEffectTag/* szTag1*/, _uint EffectBoneID /* iInt0 */, _uint iEffectTypeID, _bool bTracking/*iInt1*/)
{
...
void CE_GAzizi::Shot_MissileKey(_uint iMissilePoolID, _uint iEffectBoneID)
{
	_vector vInitPos;
	if (0 != iEffectBoneID)
	{
	...
	if (nullptr != m_pTarget)
		...
		vTargetPos = vInitPos + XMVectorSet(0.f, 1.2f, 0.f, 0.f) + XMVector3Normalize(m_pMainTransform->Get_State(CTransform::STATE_LOOK));

	m_MissilePool->Shot(vInitPos, m_pMainTransform->Get_State(CTransform::STATE_LOOK), matRot, vTargetPos);
}

void CE_GAzizi::Shot_OBBKey(_bool bOBB, _uint iAttackInfoID)
{
	m_pAttackCollider->SetActive(bOBB);
	//m_iCurAttackID = iAttackInfoID;
}

void CE_GAzizi::Shot_DissolveKey(_bool bDissolveType, _float fDissolveSpeed)
{
...
void CE_GAzizi::Shot_Echo(CTransform * pTransform, CCharacter * pTarget, _uint iNaviCellID)
{
	SetState(ACTIVE);
...
void CE_GAzizi::Change_Target(CCharacter * pActiveCharacter)
{
	m_pTarget = pActiveCharacter;
	m_pTargetTransform = m_pTarget->GetTransform();
}

void CE_GAzizi::Init_AttackInfos()
{
	ZeroMemory(&m_AttackInfo, sizeof TAGATTACK);

	m_AttackInfo.fDamageFactor = 1.7f;
	m_AttackInfo.eHitIntensity = HIT_FLY;
	m_AttackInfo.eElementType = ELMT_SPECTRA;
	m_AttackInfo.fSPGain = 0.f;
	m_AttackInfo.fTPGain = 0.f;
	m_AttackInfo.iHitEffectID = 2;
	lstrcpy(m_AttackInfo.szHitEffectTag, TEXT("GenkiDama_Boom"));
}
void CE_GAzizi::Init_Missiles()
{
	CMissilePool::MISSILEPOOLDESC tMissilePoolDesc;
	ZeroMemory(&tMissilePoolDesc, sizeof(tMissilePoolDesc));

	tMissilePoolDesc.pMissilePoolTag = TEXT("GenkiDama_Shoot2_Echo_%d");
	...
	m_MissilePool = CMissilePool::Create(m_pDevice, m_pContext, XMVectorSet(0.f, 1.2f, 0.f, 0.f), &tMissilePoolDesc);
}

void CE_GAzizi::Release_Missiles()
{
	if (nullptr != m_MissilePool)
		Safe_Release(m_MissilePool);
}

HRESULT CE_GAzizi::SetUp_ShaderResources()
{
	if (nullptr == m_pShaderCom)
		return E_FAIL;
	CGameInstance* pGameInstance = CGameInstance::GetInstance();
...
HRESULT CE_GAzizi::Setup_ShadowShaderResource()
{
	if (nullptr == m_pShaderCom)
		return E_FAIL;
	CGameInstance* pGameInstance = CGameInstance::GetInstance();
...
void CE_GAzizi::Tick_State(_double TimeDelta)
{
	//
...
HRESULT CE_GAzizi::Init_EffectBones()
{
...
void CE_GAzizi::Update_EffectBones()
{
...
CE_GAzizi * CE_GAzizi::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
{
	CE_GAzizi* pInstance = new CE_GAzizi(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed to Create : CE_GAzizi");
...
CGameObject * CE_GAzizi::Clone(void * pArg)
{
	CE_GAzizi* pInstance = new CE_GAzizi(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed to Clone : CE_GAzizi");
		Safe_Release(pInstance);
	}
...
void CE_GAzizi::OnCollisionEnter(CCollider * src, CCollider * dest)
{
	CGameMode* pGM = CGameMode::GetInstance();
...
void CE_GAzizi::OnCollisionStay(CCollider * src, CCollider * dest)
{
	CCharacter* pOpponent = dynamic_cast<CCharacter*>(dest->GetOwner());
...
void CE_GAzizi::OnCollisionExit(CCollider * src, CCollider * dest)
{
}
