...
void CRock_4::LateTick(_double TimeDelta)
{
...
HRESULT CRock_4::Load_DiffuseTexID(const _tchar * pDiffuseTexIDFilePath)
{
	...
	{
		MSG_BOX("Failed to Load Data in CRock_4 : DiffuseTexID");
		return E_FAIL;
	}
...
HRESULT CRock_4::Add_Components()
{
	/* For.Com_Renderer*/
	...
	/* For.Com_Model */
	if (FAILED(__super::Add_Component(LEVEL_ANYWHERE, SMODEL::SMD_ROCK_4,
...
HRESULT CRock_4::SetUp_ShadowShaderResources()
{
	if (nullptr == m_pShaderCom)
		return E_FAIL;
	CGameInstance* pGameInstance = CGameInstance::GetInstance();
...
CRock_4 * CRock_4::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
{
	...
	{
		MSG_BOX("Failed to Create : CRock_4");
...
CGameObject * CRock_4::Clone(void * pArg)
{
	...
	{
		MSG_BOX("Failed to Clone : CRock_4");
		Safe_Release(pInstance);
	}
