#include "stdafx.h"
#include "..\Public\Rock_2.h"

#include "GameInstance.h"
#include "GameMode.h"

CRock_2::CRock_2(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
	: CGameObject(pDevice, pContext)
{
}

CRock_2::CRock_2(const CRock_2 & rhs)
	: CGameObject(rhs)
{
}
...
void CRock_2::Tick(_double TimeDelta)
{
	__super::Tick(TimeDelta);
}

void CRock_2::LateTick(_double TimeDelta)
{
...
HRESULT CRock_2::Load_DiffuseTexID(const _tchar * pDiffuseTexIDFilePath)
{
	...
	{
		MSG_BOX("Failed to Load Data in CRock_2 : DiffuseTexID");
		return E_FAIL;
	}
...
HRESULT CRock_2::Add_Components()
{
	/* For.Com_Renderer*/
	...
	/* For.Com_Model */
	if (FAILED(__super::Add_Component(LEVEL_ANYWHERE, SMODEL::SMD_ROCK_2,
...
HRESULT CRock_2::SetUp_ShaderResources()
{
...
HRESULT CRock_2::SetUp_ShadowShaderResources()
{
	if (nullptr == m_pShaderCom)
		return E_FAIL;
	CGameInstance* pGameInstance = CGameInstance::GetInstance();
...
CRock_2 * CRock_2::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
{
	CRock_2* pInstance = new CRock_2(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed to Create : CRock_2");
...
{
	CRock_2* pInstance = new CRock_2(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed to Clone : CRock_2");
		Safe_Release(pInstance);
	}
