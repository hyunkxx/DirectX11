...
private:
	CE_Leilie(ID3D11Device* pDevice, ID3D11DeviceContext* pContext);
	CE_Leilie(const CE_Leilie& rhs);
	virtual ~CE_Leilie() = default;
...
public:
	...
	virtual void Get_AttackInfo(_uint iAttackID, TAGATTACK* pAttackInfoOut, _float* pAttackOut) override
	{
		...
		*pAttackOut = m_pEchoSystem->GetEcho(CEchoSystem::EC_IMPULSE_PREDATOR).fAttack;
	}
...
public:
	static CE_Leilie* Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext);
	...
	void CE_Leilie::OnCollisionEnter(CCollider * src, CCollider * dest) override;
	void CE_Leilie::OnCollisionStay(CCollider * src, CCollider * dest) override;
	void CE_Leilie::OnCollisionExit(CCollider * src, CCollider * dest) override;
