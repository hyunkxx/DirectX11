...
private:
	CE_Huojin(ID3D11Device* pDevice, ID3D11DeviceContext* pContext);
	CE_Huojin(const CE_Huojin& rhs);
	virtual ~CE_Huojin() = default;
...
public:
	...
	virtual void Get_AttackInfo(_uint iAttackID, TAGATTACK* pAttackInfoOut, _float* pAttackOut) override
	{
		...
		*pAttackOut = m_pEchoSystem->GetEcho(CEchoSystem::EC_FUSHION_WARRIOR).fAttack;
	}
...
public:
	static CE_Huojin* Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext);
	...
	void CE_Huojin::OnCollisionEnter(CCollider * src, CCollider * dest) override;
	void CE_Huojin::OnCollisionStay(CCollider * src, CCollider * dest) override;
	void CE_Huojin::OnCollisionExit(CCollider * src, CCollider * dest) override;
