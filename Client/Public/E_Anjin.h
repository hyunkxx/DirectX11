...
private:
	CE_Anjin(ID3D11Device* pDevice, ID3D11DeviceContext* pContext);
	CE_Anjin(const CE_Anjin& rhs);
	virtual ~CE_Anjin() = default;
...
public:
	...
	virtual void Get_AttackInfo(_uint iAttackID, TAGATTACK* pAttackInfoOut, _float* pAttackOut) override
	{
		...
		*pAttackOut = m_pEchoSystem->GetEcho(CEchoSystem::EC_RUPTURE_WARRIOR).fAttack;
	}
...
public:
	static CE_Anjin* Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext);
	...
	void CE_Anjin::OnCollisionEnter(CCollider * src, CCollider * dest) override;
	void CE_Anjin::OnCollisionStay(CCollider * src, CCollider * dest) override;
	void CE_Anjin::OnCollisionExit(CCollider * src, CCollider * dest) override;
