...
private:
	CE_GAzizi(ID3D11Device* pDevice, ID3D11DeviceContext* pContext);
	CE_GAzizi(const CE_GAzizi& rhs);
	virtual ~CE_GAzizi() = default;
...
public:
	...
	virtual void Get_AttackInfo(_uint iAttackID, TAGATTACK* pAttackInfoOut, _float* pAttackOut) override
	{
		...
		*pAttackOut = m_pEchoSystem->GetEcho(CEchoSystem::EC_ZIGZAG).fAttack;
	}
...
public:
	static CE_GAzizi* Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext);
	...
	void CE_GAzizi::OnCollisionEnter(CCollider * src, CCollider * dest) override;
	void CE_GAzizi::OnCollisionStay(CCollider * src, CCollider * dest) override;
	void CE_GAzizi::OnCollisionExit(CCollider * src, CCollider * dest) override;
