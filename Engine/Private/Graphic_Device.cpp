...
HRESULT CGraphic_Device::Ready_Graphic_Device(HWND hWnd, GRAPHIC_DESC::WIN_MODE eWinMode, _uint iWinSizeX, _uint iWinSizeY, ID3D11Device ** ppDevice_out, ID3D11DeviceContext ** ppContext_out)
{
	...
	viewPortDesc[VIEWPORT_SMALL].TopLeftX = 0;
	viewPortDesc[VIEWPORT_SMALL].TopLeftY = 0;
	viewPortDesc[VIEWPORT_SMALL].Width = 640.f;
...
HRESULT CGraphic_Device::Clear_SmallDepthStencilView()
{
	if (nullptr == m_pContext)
		return E_FAIL;

	m_pContext->ClearDepthStencilView(m_pSmallDepthStencilView, D3D11_CLEAR_DEPTH | D3D11_CLEAR_STENCIL, 1.f, 0);
...
HRESULT CGraphic_Device::Ready_SmallDepthStencilView(_uint iWinSizeX, _uint iWinSizeY)
{
	if (nullptr == m_pDevice)
		return E_FAIL;
	...
	if (FAILED(m_pDevice->CreateDepthStencilView(pDepthStencilTexture, nullptr, &m_pSmallDepthStencilView)))
		return E_FAIL;
