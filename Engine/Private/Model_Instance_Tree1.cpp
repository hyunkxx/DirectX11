#include "..\Public\Model_Instance_Tree1.h"

#include "Mesh_Instance_Tree1.h"
#include "Texture.h"
#include "Shader.h"

CModel_Instance_Tree1::CModel_Instance_Tree1(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
	: CComponent(pDevice, pContext)
{
}

CModel_Instance_Tree1::CModel_Instance_Tree1(const CModel_Instance_Tree1 & rhs)
	: CComponent(rhs)
...
HRESULT CModel_Instance_Tree1::Initialize_Prototype(const _tchar* pModelFilePath, const _tchar* pInstanceFilePath)
{
	...
	hFile = CreateFile(pInstanceFilePath,		// 파일 경로와 이름을 명시
		GENERIC_READ,								// 파일 접근 모드(쓰기 전용), GENERIC_READ(읽기 전용)
		NULL,										// 공유 방식, 파일이 열려 있는 상태에서 다른 프로세스가 오픈 할 때 허가하는 것에 대한 설정, NULL을 지정하면 공유하지 않겠다는 의미
		NULL,										// 보안 속성, NULL인 경우 기본값으로 보안 상태를 설정
		OPEN_EXISTING,								// 생성 방식, 해당 파일을 열어서 작업을 할 것인지, 새로 만들 것인지 설정(CREATE_ALWAYS : 파일이 없다면 생성, 있다면 덮어쓰기, OPEN_EXISTING : 파일이 있을 경우에만 연다)
	...
	if (INVALID_HANDLE_VALUE == hFile)
	{
		MSG_BOX("Failed to Load File In CModel_Instance_Tree1");
		...
		XMStoreFloat4x4(m_pInstanceMatrix, XMMatrixIdentity());
	}
	else
	{
		dwByte = 0;
...
HRESULT CModel_Instance_Tree1::Initialize(void * pArg)
{
	return S_OK;
}

HRESULT CModel_Instance_Tree1::SetUp_ShaderMaterialResource(CShader * pShaderCom, const char * pConstantName, _uint iMeshIndex, MytextureType eType)
{
...
HRESULT CModel_Instance_Tree1::SetUp_ShaderMaterialResource_Distinction(CShader * pShaderCom, const char * pConstantName, _uint iMeshIndex, MytextureType eType, _bool* IsDistinction)
{
	if (iMeshIndex >= m_iNumMeshes)
		return E_FAIL;
...
HRESULT CModel_Instance_Tree1::Render(_uint iMeshIndex)
{
	if (iMeshIndex >= m_iNumMeshes)
		return E_FAIL;
...
_uint CModel_Instance_Tree1::Get_TotalNumInstance()
{
...
VTXMATRIX * CModel_Instance_Tree1::Get_TotalInstanceMatrix()
{
	if (nullptr == m_Instance_Meshes[0])
		return nullptr;
...
VTXMATRIX * CModel_Instance_Tree1::Get_InstanceMatrix(_uint iInstanceNum)
{
	if (nullptr == m_Instance_Meshes[0])
		return nullptr;
...
void CModel_Instance_Tree1::Culling(_fmatrix Inverse_WorldMatrix, _float fRadius)
{
...
void CModel_Instance_Tree1::Get_PSA_To_InstanceMatrix(_uint iGetNum, SOBJECT_DESC * pOut)
{
	if (nullptr == m_Instance_Meshes[0])
		return;
...
HRESULT CModel_Instance_Tree1::Ready_Meshes(SMODELINFO* pModel, _float4x4* pInstanceMatrices)
{
	if (nullptr == pModel)
		return E_FAIL;
	...
	{
		CMesh_Instance_Tree1* pMesh = CMesh_Instance_Tree1::Create(m_pDevice, m_pContext, &pModel->s_pMeshes[i], m_iNumInstance, pInstanceMatrices);
		if (nullptr == pMesh)
			return E_FAIL;
...
HRESULT CModel_Instance_Tree1::Ready_Materials(SMODELINFO* pModel, const _tchar * pModelFilePath)
{
...
CModel_Instance_Tree1 * CModel_Instance_Tree1::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pContext, const _tchar * pModelFilePath, const _tchar* pInstanceFilePath)
{
	CModel_Instance_Tree1* pInstance = new CModel_Instance_Tree1(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype(pModelFilePath, pInstanceFilePath)))
	{
		MSG_BOX("Failed to Create : CModel_Instance_Tree1");
...
CComponent * CModel_Instance_Tree1::Clone(void * pArg)
{
	CModel_Instance_Tree1* pInstance = new CModel_Instance_Tree1(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed to Clone : CModel_Instance_Tree1");
		Safe_Release(pInstance);
	}
...
void CModel_Instance_Tree1::Free()
{
	__super::Free();
