...
PS_OUT PS_MAIN_COOLTIME(PS_IN In)
{
	PS_OUT			Out = (PS_OUT)0;
	float4 fcolor;
	fcolor = g_Texture2.Sample(LinearSampler,  float2(In.vTexUV.x ,In.vTexUV.y));
	Out.vColor = g_Texture.Sample(LinearSampler,  float2(In.vTexUV.x ,In.vTexUV.y));
	Out.vColor *= fcolor.a;
...
PS_OUT PS_WAVE(PS_IN In)
{
	...
	Out.vColor = g_Texture.Sample(LinearSampler,  float2(In.vTexUV.x + g_UV.x ,In.vTexUV.y + g_UV.y));
...
PS_OUT PS_CUT(PS_IN In)
{
	...
	Out.vColor = g_Texture.Sample(LinearSampler, g_fUV);
...
technique11 DefaultTechnique
{
			...
			pass UI_NoZWrite //11 알파discard
	{
		...
		PixelShader = compile ps_5_0 PS_MAIN();
			...
			pass UI_NoZWrite2 //12 알파discard
	{
		SetRasterizerState(RS_Default);
		SetDepthStencilState(DS_ZTest_NoZWrite, 0);
		SetBlendState(BS_AlphaBlend, float4(0.0f, 0.f, 0.f, 0.f), 0xffffffff);
